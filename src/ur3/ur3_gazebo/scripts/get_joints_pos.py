...
parser = argparse.ArgumentParser(description='UR3 arm data collection example')
...
def main():
    os.makedirs(os.path.join(assets_dir(), 'data', '{}_ur3_train_data'.format(args.tag)))
    ...
    f_joint = open(os.path.join(assets_dir(),
        'data/{}_ur3_train_data/joint_data.txt'.format(args.tag)),"a+")
    f_end_effector = open(os.path.join(assets_dir(),
        'data/{}_ur3_train_data/end_effector_data.txt'.format(args.tag)),"a+")
    iteration = 0
